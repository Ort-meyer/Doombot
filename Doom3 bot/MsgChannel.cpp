...
*/
void idMsgChannel::Init(const int id) {
...
================
*/
idBitMsg idMsgChannel::AppendMessageInfo(idBitMsg sendMessage, const int time, const idBitMsg &msg) {
    int totalLength;
    byte unsentBuffer[16384];

    sendMessage.Init(unsentBuffer, sizeof(unsentBuffer));
    sendMessage.BeginWriting();


    // write the header
    sendMessage.WriteShort(id);
    sendMessage.WriteLong(outgoingSequence);

    // write out the message data
    WriteMessageData(sendMessage, msg);
...
=================
*/
bool idMsgChannel::Process(int time, idBitMsg &msg, int &sequence) {
